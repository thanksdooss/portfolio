...
### 4차 — 문제 중심 서술과 비전공자 배려
케이스 스터디에 세 블록을 추가했다(데이터는 `src/data/projects/*.js`).
- `why` — "왜 시작했나": 전문 용어 없이 배경 3~4문장
- `glossary` — "처음 보는 용어라면": 용어 칩을 누르면 한 문장 설명이 열린다
- `pains` — "어떤 불편을, 어떻게 풀었나": 불편 → 한 일 → 확인 3단으로 문제 해결 노력을 드러낸다

새 프로젝트는 `_template.js`에 이 세 항목이 이미 들어 있으니 반드시 채운다.
