export default {
  ...
  why: '"디지털트윈"이라는 말은 흔한데, 도면과 제어 신호와 3D 화면이 실제로 어떻게 이어지는지는 잘 보이지 않습니다. 기계 전공자로서 그 연결 고리를 처음부터 끝까지 제 손으로 이어 보고 싶었습니다.',
  glossary: [
    { t: 'PLC', d: '공장 기계를 제어하는 산업용 컨트롤러. 공장의 두뇌라고 보면 됩니다.' },
    { t: '래더 (Ladder)', d: 'PLC를 프로그래밍하는 언어. 전기 회로도처럼 생겨서 사다리(ladder)라고 부릅니다.' },
    { t: 'Modbus TCP', d: '산업 장비끼리 값을 주고받는 표준 통신 규약. 공장에서 가장 널리 쓰입니다.' },
    { t: 'D 레지스터', d: 'PLC 안의 메모리 주소. 센서 값이나 모터 속도가 여기에 담깁니다.' },
    { t: '디지털트윈', d: '실제 설비와 똑같이 움직이는 가상 설비. 실물 없이 시험하고, 실물과 비교해 이상을 찾습니다.' },
  ],
  pains: [
    { p: 'Unity(3D)는 PLC의 신호를 직접 읽지 못한다. 둘 사이에 다리가 없다.', f: 'C#으로 PLC의 D 레지스터를 읽고, 라이브러리 없이 TcpListener로 Modbus TCP 서버를 직접 작성해 Unity가 클라이언트로 붙게 했습니다.', r: 'PLC 신호 → C# → Unity 가상 컨베이어가 실시간으로 함께 움직입니다.' },
    { p: '움직일 설비 모델이 없다.', f: 'Inventor로 컨베이어(판·축·풀리·벨트)와 PLC 랙을 직접 모델링했습니다. 제조사 STEP 파일은 모듈 형상에만 썼습니다.', r: '설계 → 제어 → 통신 → 시각화가 한 줄로 이어졌습니다.' },
    { p: '팀 PoC 주제가 현장에서 쓸 만한지 확신이 없었다.', f: '3개 대학 4인팀 팀장으로 주제를 세 번 고쳤습니다. 지역 중소기업 에너지 절감 → 조선소 도장·블록 공장 에너지 절감 → 최종적으로 AI 카메라 기반 안전관제.', r: '삼성 발표용 PoC 계획서로 정리했습니다.' },
  ],
  problem: [
